<template>
  <div class="list-block">
    <div
      ...
      class="grid items-center grid-cols-10 gap-8 px-4 py-16 my-16 bg-gray-100 dark-mode:bg-gray-900 md:px-8 dark-mode:bg-black dark-mode:text-white list-slice md:my-32"
    >
      ...
      <div class="grid grid-cols-1 col-span-10 row-gap-16 md:col-span-6">
        <div
          class="grid grid-cols-6 col-span-1"
          v-for="(item, index) in slice.items"
          :key="index"
        >
          <div class="col-span-2 text-4xl font-extrabold text-center">
            0{{ index + 1 }}
          </div>

          <div class="col-span-4 text-sm text">
            <h3 class="text-xl font-medium" v-if="item.title.length > 0">
    ...
    <div
      ...
      class="relative grid items-center grid-cols-10 gap-8 row-gap-16 px-4 py-16 my-16 bg-gray-100 dark-mode:text-white md:px-8 dark-mode:bg-gray-900 list md:my-32"
    >
      ...
      <template v-for="(item, index) in slice.items">
        ...
        >
          <h3 class="mb-2 text-xl font-medium" v-if="item.title.length > 0">
            {{ $prismic.asText(item.title) }}
          </h3>
          <prismic-rich-text :field="item.text" v-if="item.text.length > 0" />
          <div
